...
void Task2code( void * pvParameters  ) {
  ...
  Serial.println(xPortGetCoreID());
  SerialBT.print("Task2 is running on core ");
  SerialBT.println(xPortGetCoreID());
  SerialBT.println("\nSOFT START MODEM...");
  modemPowerOn();
  disableGPS();
  ...
  for (;;) {
    ...
    int min2 = 0;
    int sec2 = 0;
    int signal2 = ubahmaxmin(modem.getSignalQuality(), 2, 31, 1, 5);
    ...
    while (1) {
      if (modem.getGPS(&lat2, &lon2, &speed2, &alt2, &vsat2, &usat2, &accuracy2, &year2, &month2, &day2, &hour2, &min2, &sec2)) {
        ...
        //
        int signal1 = ubahmaxmin(modem.getSignalQuality(), 2, 31, 1, 5);
        float lat = 0;
        float lon = 0;
        float accuracy = 0;
        int year = 0;
        ...
        SerialBT.println("Phone: " + phone);
        SerialBT.println("\n---GSM LOCATION FIRST---\n");
        modem.getGsmLocation(&lon, &lat, &accuracy, &year, &month, &day, &hour, &min, &sec);
